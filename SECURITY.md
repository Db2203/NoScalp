...
- **Unauthenticated read endpoints.** `/api/status` and `/api/my-entries` take an `identityId` query param with no auth and return that identity's entry/order history. The `identityId` isn't guessable from an email (it's `uuidv5` of an HMAC with a server secret), but anyone who obtains it — e.g. from a leaked URL in logs/history — could read that user's history (a bounded IDOR). Production fix: bind identity to a signed HTTP-only session set at verify time and stop trusting a client-supplied id, and move the id out of the URL into a POST body/header so it stays out of logs.
